...
from MycoNet.utils.simulate_dna_data import random_DNA_data, rand_sequence
...
directory = "_".join(
    ["sim", str(seqlen), str(total_samp), str(species), str(bs_rate), str(ws_rate)]
)
print(directory)
...
if os.path.exists(directory + "/X_train.npy"):
    ...
    pd.DataFrame.from_dict(history.history).to_csv(
        directory + "/history.csv", mode="a", header=False, index=False
    )
else:
    print("generating data")
    data = random_DNA_data(
        rand_sequence(seqlen),
        nseq=total_samp,
        nlab=species,
        seqlen=seqlen,
        bs_rate=bs_rate,
