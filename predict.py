...
def predict_batch(
    model, X, batch, labels, outfile, df_labels_key_dict
):  # , df_unite_sh):
    ...
    for b in batch_list:
        ...
        df = pd.DataFrame(
            list(zip(best_labs, best_prob, lb)), columns=["SH", "prob", "org_label"]
        )  # "taxonomy"])
...
df_labels_key_dict = pd.read_csv(
    labels_key_dict, sep="\t", header=None, names=("int", "header"), index_col=0
)
df_kmer_dict = pd.read_csv(
    kmer_dict, sep="\t", header=None, names=("kmer", "int"), index_col=0
)
...
X, missing = kmer_to_int(
    [s for h, s in data],
    k=kmer,
    kmer_key=df_kmer_dict.to_dict()["int"],
    padding=True,
    max_size=maxseq,
)
...
# with strategy.scope():
# Try loading a single-file Keras archive first (model_path + ".keras").
# If that doesn't exist, try loading a SavedModel directory. For Keras 3,
# `load_model()` doesn't support legacy SavedModel directories, so wrap
# them with `TFSMLayer` when necessary.
keras_file = model_path + ".keras"
if os.path.exists(keras_file):
    model = tf.keras.models.load_model(keras_file)
else:
    # If it's a directory, attempt to load; if load_model fails, wrap with TFSMLayer
    if os.path.isdir(model_path):
        try:
            model = tf.keras.models.load_model(model_path)
        except Exception:
            # Create a Keras Model that wraps the SavedModel using TFSMLayer.
            try:
            ...
            # Determine input length from X if available
            try:
                input_len = X.shape[1]
            except Exception:
                input_len = None
            if input_len is None:
                inp = tf.keras.Input(shape=(None,), dtype=tf.float32)
            else:
                inp = tf.keras.Input(shape=(input_len,), dtype=tf.float32)
            out = layer(inp)
            model = tf.keras.Model(inputs=inp, outputs=out)
    else:
        # Last resort: try loading the path directly and let it raise if unsupported
        model = tf.keras.models.load_model(model_path)
...
predict_batch(
    ...
    labels=labels,
    outfile=outfile,
    df_labels_key_dict=df_labels_key_dict,
)  # , df_unite_sh=df_unite_sh)
